...
### Tier 3: Multi-Agent Reasoning Layer (`packages/croviq_agents`)
...
### Tier 4: Deterministic Media & Audio Pipeline (`packages/croviq_media`)
...
### Tier 5: Canonical Domain Models (`packages/croviq_domain`)
...
### Tier 6: Observability & Logging (`packages/croviq_observability`)
...
[1. Upload]
  ...
  ➔ POST /api/workspaces/{w_id}/productions/upload (Request V4 Signed GCS URL)
  ➔ Web SPA uploads raw MP4 directly to GCS bucket (media_raw)
...
[4. Safe EDL Assembly]
  ...
  ➔ Adjusts cut start/end to zero-crossing silence regions (prevents word clipping)
  ➔ Inserts 15ms audio micro-crossfades
  ➔ Generates canonical EditDecisionList (EDL) JSON

[5. Deterministic FFmpeg Render]
  POST /api/workspaces/{w_id}/productions/{p_id}/renders (RenderType: MASTER_16_9, SHORT_9_16)
  ➔ RenderService builds FFmpeg filter-graph from EDL
  ➔ Cloud Run renders final 1080p MP4 master and 9:16 Short
  ➔ Output MP4 uploaded directly to GCS media_raw bucket
