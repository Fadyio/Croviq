...
1. **Division of Labor**:
   ...
   - **Google Cloud Speech-to-Text v2 API**: Provides frame-accurate *word-level start/end time offsets* across the audio stream.
...
2. **Cut Safety & Transition Pipeline**:
   - Word start/end boundaries from Speech-to-Text v2 prevent mid-word slicing.
...
3. **Vendor-Neutral Canonical EDL**:
   - The contract between Gemini reasoning, STT alignment, and FFmpeg rendering is a typed, versioned JSON schema (`EditDecisionList`).
...
## Consequences
- Guarantees natural, broadcast-quality speech flow and video playback using dedicated Google Speech-to-Text v2 time offsets.
