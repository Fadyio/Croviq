...
CURRENT SHA: 319744490f9ae1bfee428c4fe8b6abce5baaaa79
ITERATION: 2
OPEN P0: 0
OPEN P1: 0
OPEN P2: 0
...
| **BUG-019** | P0 | Editor / State Truth | Output rows and durations only active when artifact exists | Stale/pending outputs showed as active and edited preview showed fake duration on failure | Load failed editorial run | Unconditional `isAvailable: true` and missing failure check in duration derivation | `MediaBin.tsx`, `EditorPage.tsx` | Added `isRunFailed` and `hasProposalOrEdl` checks; disable output rows on failed runs | `editor-workspace.spec.ts` | `VERIFIED_LOCAL` | `NOT_RUN` | pending | `FIXED_LOCAL` |
| **BUG-020** | P0 | Home / Truth | Dynamic calculation of retention difference | Hardcoded static strings ("58.4% vs 44.1%", "14.3%") and arbitrary confidence badge | Inspect Home insight calculations | Static strings instead of dynamic values from catalog | `channel_dashboard.py`, `OverviewView.tsx` | Dynamically calculate early/late video retention averages and deltas; removed ungrounded confidence badge | `test_channel_dashboard.py` | `VERIFIED_LOCAL` | `NOT_RUN` | pending | `FIXED_LOCAL` |
| **BUG-021** | P0 | Home / Scope | Home overview focuses on core creator insights without fake experiments | Displayed redundant Proposed Hypothesis card | Inspect Home dashboard components | Legacy experiment card remained on Home overview | `OverviewView.tsx`, `channel_dashboard.py` | Removed redundant experiment card and set `proposed_experiment=None` | `upload-flow.spec.ts` | `VERIFIED_LOCAL` | `NOT_RUN` | pending | `FIXED_LOCAL` |
| **BUG-022** | P0 | Research / Scope | Alex autonomous research derived from channel, memory & web | User-written research prompt textareas exposed | Open Alex settings -> Research tab | Legacy prompt list concept exposed in UI and schema | `AgentSettingsDrawer.tsx`, `alex.py`, `routes.py` | Refactored research to channel-driven autonomous intelligence with schedule and source policies; removed user prompt textareas | `canonical-agent-refactor.spec.ts` | `VERIFIED_LOCAL` | `NOT_RUN` | pending | `FIXED_LOCAL` |
| **BUG-023** | P0 | Projects / UX | Recent projects list provides Delete action with confirmation | No delete button for recent projects | Open New Project page | Missing delete button and confirmation modal | `RecentProjectsList.tsx`, `NewProjectPage.tsx` | Added low-emphasis Delete button with confirmation modal wired to coordinated delete endpoint | `upload-flow.spec.ts` | `VERIFIED_LOCAL` | `NOT_RUN` | pending | `FIXED_LOCAL` |
