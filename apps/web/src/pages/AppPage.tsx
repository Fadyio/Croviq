    ...
    <div className="min-h-screen bg-background text-text-primary flex flex-col font-sans selection:bg-primary/30">
      ...
      <header className="h-14 bg-surface-1 border-b border-border-subtle px-4 md:px-6 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-text-muted hover:text-text-primary hover:bg-surface-3 rounded-md transition-colors border border-transparent hover:border-border-subtle"
            title="Sign out"
          >
      ...
      {/* Main Production Home Content */}
      <main className="flex-1 p-6 md:p-10 max-w-4xl w-full mx-auto flex flex-col gap-8">
        {/* Channel Selection Header / Choice */}
        <section className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h1 className="text-xl md:text-2xl font-semibold text-text-primary">Croviq</h1>
            ...
            {selectedChannelId && (
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <span className="px-3 py-1 rounded-md bg-surface-2 border border-border-subtle text-xs text-text-secondary flex items-center gap-1.5 font-medium">
                  <Sparkles className="w-3.5 h-3.5 text-primary" />
                  <span>{SAMPLE_CHANNEL_TITLE}</span>
                </span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
            ...
            <div
              className={`p-5 rounded-lg border transition-all flex flex-col justify-between gap-4 ${
                selectedChannelId === SAMPLE_CHANNEL_ID
                  ? "bg-surface-2 border-primary/60 shadow-sm ring-1 ring-primary/30"
            ...
            >
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider bg-primary/10 text-primary border border-primary/20 rounded">
                    Active
                  </span>
                </div>
              ...
              <button
                ...
                className={`w-full py-2 px-3 text-xs font-medium rounded-md transition-colors flex items-center justify-center gap-1.5 ${
                  selectedChannelId === SAMPLE_CHANNEL_ID
            ...
            {/* Connect YouTube Card (Honest & Non-Deceptive State) */}
            <div className="p-5 rounded-lg border border-border-subtle bg-surface-1 flex flex-col justify-between gap-4 opacity-75">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm font-semibold text-text-primary">
                    <svg className="w-4 h-4 text-danger fill-current" viewBox="0 0 24 24">
        ...
        {/* What are we making? - Direct Upload Section */}
        <section className="p-6 md:p-8 rounded-lg bg-surface-1 border border-border-subtle flex flex-col gap-6">
          <div>
            <h2 className="text-lg font-semibold text-text-primary">What are we making?</h2>
          ...
          {!selectedFile ? (
            <div
              ...
              className={`border-2 border-dashed rounded-lg p-8 md:p-12 flex flex-col items-center justify-center text-center cursor-pointer transition-colors ${
                isDragOver
                  ? "border-primary bg-primary/5"
                  : "border-border-subtle hover:border-border-strong bg-surface-2/50"
              }`}
            >
              <div className="w-12 h-12 rounded-full bg-surface-3 flex items-center justify-center mb-3 text-text-secondary border border-border-subtle">
            ...
            <div className="flex flex-col gap-4 p-5 rounded-lg bg-surface-2 border border-border-subtle">
              {/* Selected File Details */}
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-md bg-surface-3 border border-border-subtle flex items-center justify-center shrink-0">
              ...
              {uploadStatus === "uploaded" && (
                <div className="p-3 bg-success/10 border border-success/20 rounded-md text-xs text-text-primary flex flex-col gap-1">
                  ...
                  {activeProductionId && (
                    <span className="font-mono text-text-secondary mt-0.5">
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                {uploadStatus === "idle" && (
                  <button
                    ...
                    className="px-4 py-2 bg-primary hover:bg-primary/90 text-white text-xs font-semibold rounded-md transition-colors flex items-center gap-1.5 shadow-sm"
                  >
                ...
                {uploadStatus === "failed" && (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={handleStartUpload}
                      className="px-4 py-1.5 bg-primary hover:bg-primary/90 text-white text-xs font-semibold rounded-md transition-colors"
                    >
                      Retry Upload
                    </button>
                  </>
        ...
        <section className="flex flex-col gap-4">
          ...
          {isLoadingProductions ? (
            <div className="p-8 rounded-lg bg-surface-1 border border-border-subtle text-center text-xs text-text-muted">
              Loading recent productions...
            </div>
          ) : productions.length === 0 ? (
            <div className="p-8 rounded-lg bg-surface-1 border border-border-subtle text-center flex flex-col items-center justify-center gap-2">
            ...
            <div className="flex flex-col gap-2.5">
              {productions.map((prod) => (
                <div
                  ...
                  className="p-4 rounded-lg bg-surface-1 border border-border-subtle hover:border-border-strong transition-colors flex items-center justify-between gap-4"
                >
                  <div className="flex items-center gap-3 min-w-0">
