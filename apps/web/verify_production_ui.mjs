...
async function main() {
  ...
  console.log("Auditing Media Mode Switching (Original -> Edited -> Studio Voice -> Original)...");
  const mediaModes = {};
  const modes = [
    {
      name: "Original",
      selector:
        '[data-testid="preview-toggle-original"], [data-testid="media-bin-row-original"], button:has-text("Original")',
    },
    {
      name: "Edited",
      selector:
        '[data-testid="preview-toggle-edited"], [data-testid="media-bin-row-edited"], button:has-text("Edited")',
    },
    {
      name: "Studio Voice",
      selector:
        '[data-testid="preview-toggle-studio-voice"], [data-testid="media-bin-row-studio_voice"], button:has-text("Studio Voice")',
    },
    {
      name: "Original_Reload",
      selector:
        '[data-testid="preview-toggle-original"], [data-testid="media-bin-row-original"], button:has-text("Original")',
    },
  ];
  ...
  for (const m of modes) {
    const btn = page.locator(m.selector).first();
    if ((await btn.count()) > 0) {
      console.log(`Switching to mode: ${m.name}...`);
      await btn.click();
      await page.waitForTimeout(2000);
      const modeData = await page.evaluate((modeName) => {
      ...
      mediaModes[m.name] = modeData;
      console.log(`Media Mode [${m.name}]:`, modeData);
    }
  }
