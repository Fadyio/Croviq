...
async function main() {
  ...
  await page.waitForURL(url => url.pathname.includes('/app') || url.pathname.includes('/productions'), { timeout: 15000 });
  ...
  const layoutAudit = await page.evaluate(() => {
    ...
    // Panel checks
    const hasMediaBin = bodyText.includes('Media') || !!document.querySelector('[data-testid="media-bin"]');
    const hasTimeline = !!document.querySelector('[data-testid="editor-timeline"]') || bodyText.includes('Timeline');
  ...
  const leoAvatar = page.locator('[data-testid="agent-avatar-leo"], button:has-text("Leo"), [aria-label*="Leo"]').first();
  ...
  console.log(`Initial Leo Prompt (${initialPrompt.length} chars): "${initialPrompt.substring(0, 80)}..."`);
  ...
  const savePromptBtn = page.locator('button:has-text("Save Prompt"), button:has-text("Save"), button:has-text("Save Changes")').first();
  ...
  await page.locator('[data-testid="agent-avatar-leo"], button:has-text("Leo"), [aria-label*="Leo"]').first().click();
  await page.waitForTimeout(1000);
  ...
  await page.locator('button:has-text("Save Prompt"), button:has-text("Save"), button:has-text("Save Changes")').first().click();
  await page.waitForTimeout(1500);
  ...
  const memoryAudit = await page.evaluate(() => {
    ...
    const hasAddBtn = buttons.some(b => ['Add', 'New', 'Create'].some(t => b.textContent?.includes(t)));
    const hasDeleteBtn = buttons.some(b => b.textContent?.includes('Delete') || b.getAttribute('aria-label')?.includes('Delete'));
    const hasEditBtn = buttons.some(b => b.textContent?.includes('Edit memory'));
    
    const headings = Array.from(document.querySelectorAll('h3, h4, h5, strong, [class*="heading"], [class*="title"]'))
    ...
    return {
      ...
      hasEditBtn,
      headings,
      memorySample: text.substring(text.indexOf('Memory') || 0, (text.indexOf('Memory') || 0) + 300)
    };
  });
  ...
  const voiceAudit = await page.evaluate(() => {
    ...
    return {
      ...
      hasPlaySample: text.includes('Play Sample') || !!document.querySelector('button:has-text("Play Sample")')
    };
  });
  console.log('Voice Audit:', JSON.stringify(voiceAudit, null, 2));

  // Test Play Sample
  const playSampleBtn = page.locator('button:has-text("Play Sample"), button:has-text("Sample")').first();
  ...
  // Close Leo modal
  const closeBtn = page.locator('button:has-text("Close"), button[aria-label="Close"], [data-testid="close-drawer"], [data-testid="close-modal"]').first();
  ...
  const mayaAvatar = page.locator('[data-testid="agent-avatar-maya"], button:has-text("Maya"), [aria-label*="Maya"]').first();
  ...
  const shortBtn = page.locator('button:has-text("Short"), button:has-text("9:16"), [data-testid="mode-short"]').first();
  let shortAudit = null;
  ...
  fs.writeFileSync('/tmp/croviq_production_proof_report.json', JSON.stringify(finalSummary, null, 2));
