"""Canonical domain models for Narration and Studio Voice artifacts."""
...
class NarrationSegment(BaseModel):
    ...
    edited_start_ms: int | None = Field(default=None, ge=0, description="Start timestamp on edited timeline in ms")
    edited_end_ms: int | None = Field(default=None, ge=0, description="End timestamp on edited timeline in ms")
    change_type: str | None = Field(default=None, description="Correction classification")
    meaning_preserved: bool = Field(default=True, description="Whether meaning is strictly preserved")
    available_duration_ms: int = Field(..., ge=0, description="Strict maximum duration budget in ms")
    original_text: str = Field(..., description="Original spoken transcript text")
    rewritten_text: str = Field(..., description="Leo's editorial rewritten text")
...
class StudioVoiceResult(BaseModel):
    ...
    edl_id: str | None = Field(default=None, description="Active EDL identifier used for lineage tracking")
    edl_version: int | None = Field(default=None, description="Active EDL version used for lineage tracking")
    corrected_script_version: str | None = Field(default=None, description="Corrected script version lineage reference")
